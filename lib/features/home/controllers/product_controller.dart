...
import 'package:efood_table_booking/features/home/domain/models/category_model.dart';
  ...
  void setCartVariationIndex(int index, int i, Product product, bool isMultiSelect) {
    if(!isMultiSelect) {
      ...
      if(!_selectedVariations[index][i] && selectedVariationLength(_selectedVariations, index) >= product.variations![index].max!) {
        showCustomSnackBarHelper('${'maximum_variation_for'.tr} ${product.variations?[index].name} ${'is'.tr} ${product.variations?[index].max}', isToast: true);
      }else {
